...
recoveryController.requestCode = async (req, res) => {
  try {
    ...
    const mailOptions = {
      ...
      text: "Usa este código pare recuperar tu cuenta",
...
recoveryController.newPassword = async (req, res) => {
  try {
    const { newPassword, confirmNuewPassword } = req.body;

    if (newPassword !== confirmNuewPassword) {
      return res.status(400).json({ message: "Password doenst match" });
    }
